...
export class FENConverter {
    ...
    public convertBoardToFEN(board: (Piece|null)[][], playerColor: Color, lastMove: LastMove|undefined, fiftyMoveRuleCounter:number, numberOfFullMoves:number): string {
        let FEN: string = "";
        for (let i = 7; i >= 0; i--) {
            let FENRow: string = "";
            let consecutiveEmptySquaresCounter: number = 0;
    ...
    private castlingAvailability(board: (Piece|null)[][]): string {
        const castlingPossibilities = (color:Color): string => {
            ...
            if (king instanceof King && !king.hasMoved) {
                ...
                const kingSideRook: Piece|null = board[rookPositionX][7];
                const queenSideRook: Piece|null = board[rookPositionX][0];
