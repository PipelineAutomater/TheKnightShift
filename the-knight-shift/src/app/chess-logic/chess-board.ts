...
import { CheckState, Color, Coords, FENChar, GameHistroy, LastMove, LegalSquares, MoveList, MoveType } from "./models";
import { Piece } from "./pieces/piece";
import { Queen } from "./pieces/queen";
import { Rook } from "./pieces/rook";
import { FENConverter } from "./FENConverter";
import { columns } from "../../../../the-knight-shift/src/app/modules/chess-board/models";

export class ChessBoard {
    private chessBoard: (Piece|null)[][];
    private chessBoardSize = 8;
    private _playerColor = Color.White; //Initially it is the white player that is playing
    private _legalSquares: LegalSquares;
    ...
    private _gameHistory: GameHistroy;
    ...
    constructor() {
        ...
        this._legalSquares = this.findLegalSquares();
    ...
    public get legalSquares(): LegalSquares {
        return this._legalSquares;
    }
    ...
    public get gameHistory(): GameHistroy {
    ...
    private isPositionLegalAfterMove(prevX: number, prevY: number, newX: number, newY: number): boolean {
        ...
        const isPositionLegal = !this.isInCheck(piece.color, false);
        this.chessBoard[prevX][prevY] = piece;
        this.chessBoard[newX][newY] = newPiece;
        return isPositionLegal;
    }

    private findLegalSquares(): LegalSquares {
        const legalSquares: LegalSquares = new Map<string, Coords[]>();
        for (let x = 0; x < this.chessBoardSize; x++) {
            for (let y = 0; y < this.chessBoardSize; y++) {
                const piece: Piece|null = this.chessBoard[x][y];
                if (!piece || piece.color !== this._playerColor) continue;
                const pieceLegalSquares: Coords[] = []; 
                for (const {x: dx, y: dy} of piece.directions) {
                    let newX: number = x + dx;
                    let newY: number = y + dy;
                    if(!this.areCoordsValid(newX, newY)) continue;
                    ...
                    if (piece instanceof Pawn) {
                        ...
                        if ((dy === 1 || dy === -1) && (!newPiece || newPiece.color === piece.color)) continue;
                    }
                    if (piece instanceof Pawn || piece instanceof Knight || piece instanceof King) {
                        if(this.isPositionLegalAfterMove(x, y, newX, newY)) pieceLegalSquares.push({x: newX, y:newY});
                    } else {
                        while (this.areCoordsValid(newX, newY)) {
                ...
                if (piece instanceof King) {
                    if (this.canCastle(piece, true)) pieceLegalSquares.push({x, y: 6});
                    if (this.canCastle(piece, false)) pieceLegalSquares.push({x, y:2});
                }
                if (piece instanceof Pawn && this.canCaptureEnPassant(piece, x, y)) 
                    pieceLegalSquares.push({x: x + (piece.color === Color.White ? 1 : -1), y: this._lastMove!.prevY}); 
                if (pieceLegalSquares.length) {
                    legalSquares.set(x + "," + y, pieceLegalSquares);
                }
            }
        }
        return legalSquares;
    ...
    private canCaptureEnPassant(pawn: Pawn, pawnX: number, pawnY: number): boolean {
        //takeaway: at the beginning of methods, always check for values that could break the rest of the code. If you are using a variable that could be null, for example, the fact that you haven't checked if it is null should come to your mind as you use the variable.
        ...
        pawnX !== currX || 
        Math.abs(pawnY - currY) !== 1) return false;
        ...
        const isPositionLegal: boolean = this.isPositionLegalAfterMove(pawnX, pawnY, pawnNewPositionX, pawnNewPositionY);
    ...
    private canCastle (king: King, kingSideCastle: boolean): boolean {
        ...
        return this.isPositionLegalAfterMove(kingPositionX, kingPositionY, kingPositionX, firstNextKingPositionY) 
        && this.isPositionLegalAfterMove(kingPositionX, kingPositionY, kingPositionX, secondNextKingPositionY);
    }

    public move(prevX: number, prevY: number, newX: number, newY: number, promotedPieceType: FENChar|null): void {
        if (this._isGameOver) throw new Error("Game is over, you can't play a move.");
        if (!this.areCoordsValid(prevX, prevY) || !this.areCoordsValid(newX, newY)) return;
        const piece: Piece|null = this.chessBoard[prevX][prevY];
        if (!piece || piece.color !== this._playerColor) return;
        
        const pieceLegalSquares: Coords[]|undefined = this._legalSquares.get(prevX + "," + prevY);
        if (!pieceLegalSquares || !pieceLegalSquares.find(coords => coords.x === newX && coords.y === newY)) throw new Error("Square is not legal");
        //I think swapping conditionals A and B would more efficiently take advantage of short-circuit evaluation.
        if ((piece instanceof Pawn || piece instanceof King || piece instanceof Rook) && !piece.hasMoved) piece.hasMoved = true;
        ...
        const legalSquares: LegalSquares = this.findLegalSquares();
        if (this._checkState.isInCheck) moveType.add(!legalSquares.size ? MoveType.CheckMate : MoveType.Check);
        else if (!moveType.size) moveType.add(MoveType.BasicMove);
        this.storeMove(promotedPieceType);
        this.updateGameHistory();
        this._legalSquares = legalSquares;
    ...
    private promotedPiece(promotedPieceType: FENChar): Knight|Bishop|Rook|Queen {
        ...
        return new Queen(this._playerColor); //Better that this is the first conditional because most of the time people promote to Queen
    }
    ...
    private playerHasOnlyBishopsWithSameColorAndKing(pieces: {piece: Piece, x: number, y: number}[]): boolean {
        ...
        const areAllBishopsOfSameColor = new Set(bishops.map(bishop => ChessBoard.isSquareDark(bishop.x, bishop.y))).size === 1; //could declare the variable to be a boolean here
    ...
    private insufficientMaterial(): boolean {
        ...
        if (whitePieces.length === 1 && blackPieces.length === 1) return true;
        else if (whitePieces.length === 1 && blackPieces.length === 2) return blackPieces.some(piece => piece.piece instanceof Knight || piece.piece instanceof Bishop);
        else if (whitePieces.length === 2 && blackPieces.length === 1) return whitePieces.some(piece => piece.piece instanceof Knight || piece.piece instanceof Bishop);
            ...
            if (whiteBishop && blackBishop) {
                ...
                return areBishopsOfSameColor; //Better way to do this without a variable and without calculating the same thing twice sometimes by storing isSquareDark return values in a variable.
            }
        }
    ...
    private startingPieceCoordsNotation(): string {
        ...
        const samePieceCoords: Coords[] = [{x: prevX, y: prevY}];
        for (let x = 0; x < this.chessBoardSize; x++) {
            for (let y = 0; y < this.chessBoardSize; y++) {
                const piece: Piece|null = this.chessBoard[x][y];
                if (!piece || currX === x && currY === y) continue;
                if (piece.FENChar === currPiece.FENChar) {
                    const legalSquares: Coords[] = this._legalSquares.get(x + "," + y) || [];
                    const pieceHasSameTargetSquares: boolean = legalSquares.some(coords => coords.x === currX && coords.y === currY);
                    if (pieceHasSameTargetSquares) samePieceCoords.push({x, y});
                }
            }
        }
        if (samePieceCoords.length === 1) return "";
        const piecesFile = new Set(samePieceCoords.map(coords => coords.y));
        const piecesRank = new Set(samePieceCoords.map(coords => coords.x));
        if (piecesFile.size === samePieceCoords.length) return columns[prevY];
        if (piecesRank.size === samePieceCoords.length) return String(prevX + 1);
