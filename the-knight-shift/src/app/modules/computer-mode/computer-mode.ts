...
export class ComputerMode extends ChessBoardComponent implements OnInit, OnDestroy{
  ...
  private FEN:string;
  constructor(private stockfishService:StockfishService, private router: Router){
  ...
  public override ngOnInit(): void {
    super.ngOnInit();
    this.router.events.pipe(filter(event => event instanceof NavigationEnd)).subscribe(() => this.chessBoardService.reset());
    ...
    const chessBoardStateSubscription$: Subscription = this.chessBoardService.chessBoardState$.subscribe({
      next: async (FEN: string) => {
        ...
        while (!response) {
          try {
            response = await firstValueFrom(this.stockfishService.getBestMove(FEN).pipe(retry({count: 5, delay: (_, retryCount) => {const waitTime = Math.pow(2, retryCount + 1) * 1; return timer(waitTime)}})));
          } catch(e) {
